...
router.post('/saveOrder', function (req, res, next) {
    ...
    var param = req.body;
    var orderUserId = param.orderUserId;
    var noticeId = param.noticeId;
    var rewardMoney = param.rewardMoney; //赏金
    ...
    UserModel.findOne({
        ...
        if (err) {
            ...
            totalmoney = doc.totalmoney - rewardMoney;
            if (totalmoney < 0) {
                resData.code = 1;
                resData.returnValue = {
                    message: '余额不足请充值',
                ...
                orderEntity.save(function (err, docs) {
                    if (err) {
                        resData.code = 5;
                        resData.errorReason = RestResult.SERVER_EXCEPTION_ERROR_DESCRIPTION;
                        res.send(resData);
                    } else {
                        NoticeModel.updateOne({
                            _id: noticeId
                        }, {
                            $set: {
                                status: 1,
